...
class SimpleCNN(nn.Module):
    def __init__(self):
        ...
        self.fc2 = nn.Linear(64, 10, bias=False)  # Output layer
...
class SimpleCNN_deep(nn.Module):
    def __init__(self):
        ...
        self.conv1 = nn.Conv2d(1, 6, kernel_size=3, stride=1, padding=1, bias=False)  # Convolutional layer
        self.pool = nn.MaxPool2d(kernel_size=2, stride=2, padding=0)  # Max pooling
        self.conv2 = nn.Conv2d(6, 6, kernel_size=3, stride=1, padding=1, bias=False)  # Convolutional layer
        self.fc1 = nn.Linear(6 * 14 * 14, 64, bias=False)  # Fully connected layer
        self.fc2 = nn.Linear(64, 10, bias=False)  # Output layer
...
class SimpleCNN_wide(nn.Module):
    def __init__(self):
        ...
        self.pool = nn.MaxPool2d(kernel_size=2, stride=2, padding=0)  # Max pooling
        self.fc1 = nn.Linear(6 * 14 * 14, 128, bias=False)  # Fully connected layer
        # self.bn2 = nn.BatchNorm1d(128, affine=False)  # Batch normalization
        self.fc2 = nn.Linear(128, 10, bias=False)  # Output layer
...
class Walker:
    def __init__(self, limits, params, device, rank, model_class=None):
        ...
        self.all_data = torch.cat([self.train_data, self.test_data], dim=0)
        self.all_data = self.all_data.to(self.dtype)
        ...
        self.model.compile(mode="max-autotune") # PyTorch 2.0 compile for speedup
        ...
        if os.path.exists(v_filename):
            ...
            self.need_initialization = torch.zeros(1, device='cpu')
    ...
    def save(self):
        ...
        if not self.need_initialization:
            ...
            np.savez(samples_filename, h=self.h, log_g=self.log_g, it=self.it, log_f=self.log_f,
    ...
    def get_accuracies(self):
        with torch.no_grad():
            out_all = self.model(self.all_data)

        _, pred = out_all.topk(1)

        pred_train = pred[:self.train_data.size(0)]
        acc = (pred_train.squeeze() == self.train_target).sum().item()
        # e = int((acc/self.n_train)*(self.bins_e-1))
        e = acc  # maximum granularity

        pred_test = pred[self.train_data.size(0):]
        acc = (pred_test.squeeze() == self.test_target).sum().item()
        q = int(((acc - 1) / self.n_test) * self.bins_q)
    ...
    def step(self):
        ...
        if (self.h > self.flatness * self.h.mean()).all():
            self.h = 0 * self.h
...
def exchange(walker, paired_rank, params):
    rank = walker.rank
    ...
    if rank > paired_rank:
        ...
        if pe < walker.e_min or pe > walker.e_max or pq < walker.q_min or pq > walker.q_max:
            ...
            if is_there_interchange:  # let's interchage!
                ...
                paired_v = torch.zeros(walker.num_spins, device="cpu", dtype=params.dtype)
